...
const Modal = ({ open, onClose}: { open: boolean, onClose: () => void}) => {
  return (
    <div
      className={`fixed duration-700 ease-in-out text-[#A9B2EF] top-0 lg:w-[390px] w-full h-screen bg-[#09155A] rounded-t-xl transition-all  ${open ? 'translate-y-0' :'translate-y-full'}`}
    >
      ...
      <div className="p-4 flex flex-col gap-3 py-10">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div>
            <p className="mb-1 text-[14px]">The letter T, O, S is in the word, but in the wrong place</p>
            <Solution word={word} solution={solution} />
          </div>
          <div>
            <p className="mb-1 text-[14px]">The letter S,H,O is in the word and in the correct place</p>
            <Solution word={word2} solution={solution} />
          </div>
          <div>
            <p className="mb-1 text-[14px]">The letter SHOTS is in the word and in the correct place</p>
...
const Solution = ({ word, solution}: any) => {
  return (
